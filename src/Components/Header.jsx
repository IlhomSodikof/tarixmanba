    ...
    <>
      <header>
        <div className="header-top-box">
          <Link className='logo-box'><img className='logo' src={Logo} alt="Logo" /></Link>
          <div className="search-box-bigs">
            <div className="search-box">
              <input className='search-inp' type="search" placeholder="Search" required /> <span className="search-btn"><IoIosSearch /></span>
            </div>
            <div className='functionale'>

              <div className="ob-havo">Toshkent 18+ ... 20+ C</div>
              <div className='dark-m' onClick={() => { setMaxDrop(!maxDrop) }} >
                <GiSunglasses /> <span>Maxsus imkonyatlar</span>
                <motion.ul className={maxDrop ? "max-imkonyat" : ""}
                  ref={valRefMax}
                  initial={{ scale: 0 }}
        ...
        <nav>
          <div className="nav-box">
            <div className="nav-drop" onClick={() => { setDrop(!drop) }}>
              <Link className='link'>Manbalar</Link>
              <motion.div className='arr' animate={{ rotate: drop ? 1080 : 540 }}
                transition={{
                  type: "spring",
                  stiffness: 560,
                  damping: 50
            ...
            <div className="nav-menu">
              <Link className='link' >Yangiliklar</Link>
            </div>
            <div className="nav-menu">
            ...
            <div className="nav-menu nav-login" onClick={() => { setLoginDrop(!loginDrop) }}>
              <Link className='link' >Login</Link>
              <motion.div className='arr' animate={{ rotate: loginDrop ? 1080 : 540 }}
                transition={{
                  type: "spring",
                  stiffness: 560,
                  damping: 50
              ...
              <motion.ul className={loginDrop ? "menu-login" : ""}
                ref={valRefLogin}
                initial={{ scale: 0 }}
              ...
              >
                <motion.li className="login-item" whileHover={{ x: 15, opacity: 0.5 }}  >
